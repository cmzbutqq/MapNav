...
bool isPointInCircumcircle(const QPointF &p, const QPointF &a, const QPointF &b,
                           const QPointF &c) {
    double ax = a.x() - p.x();
    double ay = a.y() - p.y();
    double bx = b.x() - p.x();
...
Map::Map() {
    /*
构造函数中随机生成10000到20000个点，然后使用Delaunay
三角剖分生成边，然后利用lambda 函数遍历产生的三角形集合将所有的边加入到Map
的成员变量QVector<Edge> edges中;在定义边Edge的时候虽然有from 和to，但是在加入到
edges时，按照无向图的方法加入，所以每条边只会 加入一次，并且每个结点的QSet<int>
connectedEdges能够正确更新与它相连接的边的id，通过edges可以访问每一条边，通过vertices可以访问
每个节点
    */
    ...
    for (int i = 0; i < Vertexcount; i++) {
        ...
        for (const auto &triangle : triangulation) {
            if (isPointInCircumcircle(vertices[i].position,
                                      vertices[triangle.a].position,
                                      vertices[triangle.b].position,
                                      vertices[triangle.c].position)) {
        ...
        for (const auto &triangle : badTriangles) {
            for (int j = 0; j < 3; j++) {
                ...
                for (const auto &otherTriangle : badTriangles) {
                    if (triangle.a == otherTriangle.a &&
                        triangle.b == otherTriangle.b &&
                        triangle.c == otherTriangle.c) {
                        continue;
                    }
                    if ((a == otherTriangle.a && b == otherTriangle.b) ||
                        (a == otherTriangle.b && b == otherTriangle.c) ||
                        (a == otherTriangle.c && b == otherTriangle.a)) {
                        isShared = true;
                        break;
                    }
                }
        ...
        for (const auto &triangle : badTriangles) {
            triangulation.erase(std::remove(triangulation.begin(),
                                            triangulation.end(), triangle),
                                triangulation.end());
        }
    ...
    for (const auto &triangle : triangulation) {
        auto addEdge = [this, &addedEdges](int from, int to) {
            ...
            if (addedEdges.find(edge) == addedEdges.end()) {
                ...
                double len = std::sqrt((p1.x() - p2.x()) * (p1.x() - p2.x()) +
                                       (p1.y() - p2.y()) * (p1.y() - p2.y()));
        ...
        /*
auto addEdge = [this, &addedEdges](int from, int to) {... };：定义一个名为
addEdge 的 lambda 函数。 [this, &addedEdges] 是捕获列表，this
捕获当前对象的指针，以便在 lambda 函数中访问类的成员（如 vertices 和 edges），
 &addedEdges 以引用的方式捕获 addedEdges 集合，这样在 lambda
函数中可以修改它。int from 和 int to 是函数的参数，表示边的两个端点的索引。
std::pair<int, int> edge = std::make_pair(std::min(from, to), std::max(from,
to));：为了确保无论边的两个端点以何种顺序传入，都能得到一致的表示，
将较小的索引作为 pair 的第一个元素，较大的作为第二个元素。这样可以避免将 (1, 2)
和 (2, 1) 视为不同的边。 if (addedEdges.find(edge) == addedEdges.end()) {...
}：使用 addedEdges.find(edge) 在已添加边的集合中查找当前边 edge。 如果 find
函数返回 addedEdges.end()，说明当前边尚未添加过，进入 if 块。 QPointF p1 =
vertices[from].position; 和 QPointF p2 =
vertices[to].position;：获取边的两个端点的坐标。 double len = std::sqrt((p1.x()
- p2.x()) * (p1.x() - p2.x()) + (p1.y() - p2.y()) * (p1.y() -
p2.y()))计算边的长度。 edges.push_back(Edge(Edgecount++, from, to,
len));：创建一个新的 Edge 对象，包含边的 ID（Edgecount）、起点 from、终点 to
和长度 len，并将其添加到 edges 容器中。 vertices[from].addEdge(Edgecount - 1);
和 vertices[to].addEdge(Edgecount - 1);：将当前边的 ID
添加到边的两个端点对应的顶点对象的 connectedEdges 集合中，
addedEdges.insert(edge);：将当前边添加到 addedEdges
集合中，以便后续检查边是否已经添加过。
         */
    ...
    vertices.resize(
        Vertexcount); // vertices
                      // 中元素的顺序与最初随机生成顶点时的顺序保持一致，并且每个顶点的连接信息都已更新，构成了一个通过
                      // Delaunay 三角剖分得到的连通图。
